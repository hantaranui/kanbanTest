/* widget.js — Logique principale du Kanban LaSuite.coop
   Sources : Varamil (structure, DnD, getChoices/colors)
             Élodie Gateau (setCursorPos, renderFieldValue)
             Éric Couillard (RemoveRecord + confirm)
*/

/* ── État global ──────────────────────────────────────────────── */
let _records    = [];      // snapshot onRecords
let _colsMeta   = [];      // snapshot _grist_Tables_column
let _tableId    = null;    // ID de la table source
let _statusColId = null;   // colId mappé comme STATUT
let _mappings   = {};      // { name: colId } depuis grist.ready columns
...
grist.ready({
  ...
  columns: [
    ...
    { name: 'CHAMP_2',     title: 'Champ 2',        type: 'Any',     optional: true, description: 'Champ optionnel affiché sur les cartes' },
...
async function _loadColsMeta() {
  try {
    ...
    _tableId = table._tableId ?? table.tableId ?? null;
    ...
    if (tableRef !== null) {
      _colsMeta = colsData.id
        ...
        .filter(c => colsData.parentId[i] === tableRef ||
                     /* fallback si parentId non dispo */ true)
        .filter(c => c.colId && !c.colId.startsWith('gristHelper'));
    }
...
grist.onRecords(async (records, mappings) => {
  ...
  if (_colsMeta.length === 0) await _loadColsMeta();
...
async function _render() {
  ...
  choices.forEach(choice => {
    const col = _buildColumn(choice, statusMeta);
  ...
  _records.forEach(rec => {
    ...
    if (!zone) return;
    const card = _buildCard(rec);
    zone.appendChild(card);
    _updateCounter(zone.closest('.ls-col'));
  });

  /* Activation SortableJS si pas readonly */
        ...
        onEnd: async (evt) => {
          ...
          try {
            await grist.docApi.applyUserActions([[
              'UpdateRecord', _tableId, recId,
              { [_statusColId]: newStatut }
            ]]);
          } catch (e) {
            console.error('[kanban] UpdateRecord:', e);
          }
  ...
  board.querySelectorAll('.ls-col').forEach(col => {
    const key = 'ls_col_' + col.dataset.colId;
    if (localStorage.getItem(key) === 'collapsed') col.classList.add('collapsed');
    _updateCounter(col);
  });
}
...
function _buildColumn(choice, statusMeta) {
  ...
  /* Bouton ajouter (si pas readonly) */
  if (!Config.isReadonly()) {
...
function _buildCard(rec) {
  ...
  titleEl.textContent = titleVal || '(sans titre)';
  ...
  const fieldMappings = ['CHAMP_2','CHAMP_3','CHAMP_4','CHAMP_5','CREE_LE','CREE_PAR'];
  fieldMappings.forEach(name => {
  ...
  /* Bouton supprimer (si pas readonly) */
  if (!Config.isReadonly()) {
    ...
    delBtn.addEventListener('click', async (e) => {
      e.stopPropagation();
      const titre = titleVal || `carte #${rec.id}`;
      if (!confirm(`Supprimer "${titre}" ? Cette action est irréversible.`)) return;
      try {
        await grist.docApi.applyUserActions([[
          'RemoveRecord', _tableId, rec.id
        ]]);
      } catch (err) {
        console.error('[kanban] RemoveRecord:', err);
      }
    });
    card.appendChild(delBtn);
  }

  /* Clic → sélectionne la ligne → fiche liée Grist se met à jour */
...
async function _addRecord(statut) {
  ...
  try {
    await grist.docApi.applyUserActions([[
      'AddRecord', _tableId, null,
      { [_statusColId]: statut }
    ]]);
  } catch (e) {
    console.error('[kanban] AddRecord:', e);
  }
}
...
function _showConfig() {
  ...
  Config.render(cfgDiv);
}

/* ── Helpers ──────────────────────────────────────────────────── */

/* Extraire les choix depuis les widgetOptions de la colonne */
function _getChoices(colMeta) {
  if (!colMeta) return [];
  const wo = colMeta.widgetOptions
    ? (typeof colMeta.widgetOptions === 'string'
  ...
  if (wo.choicesById) {
    return Object.values(wo.choicesById).map(c => ({
      label: c.label ?? c.value ?? '',
    }));
  }
  if (Array.isArray(wo.choices)) {
    return wo.choices.map(c =>
      typeof c === 'string' ? { label: c } : { label: c.label ?? c.value ?? '' }
    );
  }
  return [];
}
